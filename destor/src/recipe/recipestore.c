...
void append_file_recipe_meta(struct backupVersion* b, struct fileRecipeMeta* r) {
	...
	if(sizeof(fid) + sizeof(recipe_offset) + sizeof(r->chunknum) + sizeof(r->filesize) > metabufsize - b->metabufoff){
		/* buf is full */
		fwrite(b->metabuf, b->metabufoff, 1, b->metadata_fp);
		b->metabufoff = 0;
	}
	...
	memcpy(b->metabuf + b->metabufoff, &fid, sizeof(fid));
	b->metabufoff += sizeof(fid);
	memcpy(b->metabuf + b->metabufoff, &recipe_offset, sizeof(recipe_offset));
	b->metabufoff += sizeof(recipe_offset);
...
struct fileRecipeMeta* read_next_file_recipe_meta(struct backupVersion* b) {
	...
	fread(&fid, sizeof(fid), 1, b->metadata_fp);
	sprintf(filename, "%ld", fid);
	struct fileRecipeMeta* r = new_file_recipe_meta(filename);
	fread(&r->offset, sizeof(r->offset), 1, b->metadata_fp);
	printf("read %ld, offset=%ld ", fid, r->offset);
